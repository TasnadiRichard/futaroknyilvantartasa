document.addEventListener("DOMContentLoaded",function(){
    ...
    createButton.addEventListener("click", async function () {
        const baseUrl ="http://localhost/futaroknyilvantartasa/futarokbackend/index.php?futarok";
    ...
    readButton.addEventListener("click", async function(){
        const baseUrl ="http://localhost/futaroknyilvantartasa/futarokbackend/futarok/index.php?futarok";
        let response = await fetch(baseUrl);
        if(response.ok){
            let data = await response.json();
            futarListazas(data);
        }else{
            console.error("Hiba a szerver válasza során")
        }
